...
function(GitStateChangedAction _state_as_list)
    ...
    configure_file("${GenVersion_IN}" "${GenVersion_OUT}" @ONLY)
endfunction()
...
function(GetGitState _working_dir _state)
    ...
    # Get preconf hash
    file(MD5 ${GenVersion_IN} _pre_conf_file_hash)

    # Get project version
    if(NOT ${GenVersion_VERSION_FILE} EQUAL "")
        file(STRINGS ${GenVersion_VERSION_FILE} _project_version)
    elseif(NOT "${GenVersion_VERSION_STRING}" EQUAL "")
        set(_project_version ${GenVersion_VERSION_STRING})
    else()
        set(_project_version "unknown")
    endif()
...
function(SetupGitMonitoring)
    add_custom_target(check_git_repository
        ALL
        DEPENDS ${GenVersion_IN}
        BYPRODUCTS ${GenVersion_OUT}
        ...
        COMMAND
            ...
            -DGenVersion_IN=${GenVersion_IN}
            -DGenVersion_OUT=${GenVersion_OUT}
            -DGenVersion_VERSION_FILE=${GenVersion_VERSION_FILE}
            -DGenVersion_VERSION_STRING=${GenVersion_VERSION_STRING}
...
function(GenVersion)
    SET(options)
    set(oneValueArgs IN OUT VERSION_FILE VERSION_STRING)
    set(multiValueArgs)
    cmake_parse_arguments(GenVersion "${options}" "${oneValueArgs}" "${multiValueArgs}" ${ARGN})

    if(NOT EXISTS ${GenVersion_IN})
        message(FATAL_ERROR "Preconfigure file doesn't exists")
    endif()

    if(${GenVersion_OUT} EQUAL "")
        message(FATAL_ERROR "OUT shouldn't be empty")
    endif()

    if(NOT ${GenVersion_VERSION_FILE} EQUAL "" AND NOT EXISTS ${GenVersion_VERSION_FILE})
...
if(_BUILD_TIME_CHECK_GIT)
    GenVersion(
            IN
                ${GenVersion_VERSION_IN}
            OUT
                ${GenVersion_VERSION_OUT}
            VERSION_FILE
                ${GenVersion_VERSION_FILE}
            VERSION_STRING
                ${GenVersion_VERSION_STRING}
    )
endif()
